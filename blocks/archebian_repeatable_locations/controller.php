<?php
namespace Concrete\Package\ArchebianRepeatableLocations\Block\ArchebianRepeatableLocations;
...
{
    protected $btTable = 'btArchebianRepeatableLocations';
    protected $btExportTables = array('btArchebianRepeatableLocations', 'btArchebianRepeatableLocationsItem');
    ...
    {
        return t('Multi location gmap');
    }

    public function getBlockTypeName()
    {
        return t('Repeatable Locations');
    }
    ...
    public function getSearchableContent()
    {
        ...
        $db = Database::get();
        $v = array($this->bID);
        $q = 'select * from btArchebianRepeatableLocationsItem where bID = ?';
        $r = $db->query($q, $v);
        foreach ($r as $row) {
            $content .= $row['title'].' ';
    ...
    public function edit()
    {
        ...
        $this->requireAsset('redactor');
        $db = Database::get();
        $query = $db->GetAll('SELECT * from btArchebianRepeatableLocationsItem WHERE bID = ? ORDER BY sortOrder', array($this->bID));
        $this->set('rows', $query);
    }
    ...
    {
        $db = Database::get();
        $r = $db->GetAll('SELECT * from btArchebianRepeatableLocationsItem WHERE bID = ? ORDER BY sortOrder', array($this->bID));
    ...
    public function duplicate($newBID)
    {
        ...
        while ($row = $r->FetchRow()) {
            $db->execute('INSERT INTO btArchebianRepeatableLocationsItem (bID, fID, linkURL, title, description, sortOrder, internalLinkCID) values(?,?,?,?,?,?,?)',
                array(
                    $newBID,
                    $row['fID'],
                    $row['linkURL'],
    ...
    {
        $db = Database::get();
        $db->delete('btArchebianRepeatableLocationsItem', array('bID' => $this->bID));
        parent::delete();
    }
    ...
    public function save($args)
    {
        ...
        $db = Database::get();
        $db->execute('DELETE from btArchebianRepeatableLocationsItem WHERE bID = ?', array($this->bID));
        parent::save($args);
        if (isset($args['sortOrder'])) {
            ...
            while ($i < $count) {
                ...
                $db->execute('INSERT INTO btArchebianRepeatableLocationsItem (bID, fID, title, description, sortOrder, linkURL, internalLinkCID) values(?, ?, ?, ?,?,?,?)',
                    array(
                        $this->bID,
                        intval($args['fID'][$i]),
                        $args['title'][$i],
